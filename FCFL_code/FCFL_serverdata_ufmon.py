...
from dataset_process.load_dataset import test_train_allocation, test_train_mixture, server_client_split, server_client_split, test_train_split, mnist_iid, mnist_noniid, cifar_iid, cifar_noniid, fmnist_iid, fmnist_noniid  # 引入了三种iid与non-iid数据库类型
...
if __name__ == '__main__':
    ...
    for iter in range(args.epochs):
        ...
        if iter == 0:
            ...
            for i in range(args.num_users):
                uf[i] = Acc_global - Acc_local_list[i]
            # print(uf)
            # print(min(uf))
            # 更新Q
            alpha = 1.0
