...
from dataset_process.load_dataset import test_train_allocation, test_train_mixture, test_train_split, mnist_iid, mnist_noniid, cifar_iid, cifar_noniid, fmnist_iid, fmnist_noniid  # 引入了三种iid与non-iid数据库类型
...
from client_selection.sampling_by_Q import sample_by_Q_prob,sample_by_Q_top, is_all_zero
...
"""
select the top-m clients with highest Q   # 可能会导致经常选择弱势客户端，从而影响全局性能
...
if __name__ == '__main__':
    ...
    if args.dataset == 'mnist':
        ...
        if args.iid:
            dict_users = mnist_iid(x_all, args.num_users)  # 为用户分配iid数据
        else:
    ...
    if args.dataset == 'synthetic':
        ...
        label_test = {}
        for i in dict_users:  
            image[i], label[i] = split(x_all, y_all, dict_users_train[i])
            image_test[i], label_test[i] = split(x_all, y_all, dict_users_test[i])
    ...
    for iter in range(args.epochs):
        ...
        if iter == 0:
            ...
            alpha = 0.1
            Q_value = []
            weights_all_clients = [0] * args.num_users
            # print(weights)
            ...
            # 选择客户端
            m = max(int(args.frac * args.num_users), 1)
            idxs_users = sample_by_Q_prob(m, Q_value)  # 根据Q选取top-m个客户端
            # print("prob:")
            # print(idxs_users)
            # idxs_users_top = sample_by_Q_top(m, Q_value)  # 根据Q选取top-m个客户端
            # print("top:")
            # print(idxs_users_top)
